...
get_appropriation_files <- function() {
  ...
  data <- fy23_appropriation %>%
    full_join(fy22_appropriation, by = c("26-Digit Account Number", "15-Digit Account Number")) %>%
    ...
    mutate(
      ...
      `DetailedFundID` = case_when(
        TRUE ~ `DetailedFund ID`,
