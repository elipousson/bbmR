...
import_tech_tables <- function(files, path = "((?<=C:/Users/sara.brumfield2/OneDrive - City Of Baltimore/FY2024 Planning/03-TLS-BBMR Review/Agency Analysis Tools/FY24 Change Table ).+(?=.xlsx))") {
  for (file in files) {
    ...
    sheets <- stats::na.omit(str_extract(excel_sheets(file), "\\d{3}"))
    df <- data.frame()
    for (s in sheets) {
      z <- read_excel(file, s) %>%
...
consolidate_tables <- function(df = x, tab_name = a, type, file_name = file_path, save = TRUE) {
  ...
  header_style <- createStyle(
    fgFill = "white", border = "TopBottomLeftRight",
    borderColour = "black", textDecoration = "bold", fontColour = "darkblue",
    wrapText = TRUE
  )

  style <- createStyle(
    fgFill = "darkblue", border = "TopBottomLeftRight",
  ...
  style_rows2 <- which(x$`Change Table (GF Only)` %in% c(
    "FY2023 Adopted", "CLS Adjustments", "Request Adjustments",
    "TLS Adjustments", "FinRec Adjustments", "BoE Adjustments",
    "Council Adjustments", "FY2024 Budget"
  )) + 1
